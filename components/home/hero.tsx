...
export default function Hero() {
  return (
    <motion.div className="mt-20 mx-auto px-6 flex flex-col" initial="hidden" animate="visible" variants={{ hidden: {}, visible: { transition: { staggerChildren: 0.2 } } }}>
      
      {/* ACMW Title */}
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease: 'easeOut' }}>
        <BlurText text="ACMW." className="text-maroon font-heading text-8xl" animateBy="letters" />
      </motion.div>
      ...
      <motion.p className="text-black/75 max-w-prose mt-5" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.6 }}>
        ACMW is UMN's chapter of the Association for Computing Machinery–Women, supporting women and non-binary students in tech through community, mentorship, and career-building events.
      </motion.p>
