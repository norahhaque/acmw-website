  ...
  return (
    <div className="flex flex-col lg:flex-row-reverse items-start lg:items-center justify-between gap-10 mt-40 lg:ml-12">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 w-full lg:w-3/4 px-10">
        <StatCard icon={<Users />} value={150} label="Active Members" />
        <StatCard icon={<CalendarDays />} value={6} label="Semester Events" />
        <StatCard icon={<Handshake />} value={15} label="Corporate Partners" />
        <StatCard icon={<UserCog />} value={10} label="Leadership Roles Open to Members" />
      </div>
    </div>
  )
}
